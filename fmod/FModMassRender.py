...
for filepath in list(Path(frontier).rglob("*.fmod")):
    ...
    space, area = next(
        ((space, area) for area in bpy.context.screen.areas if area.type == 'VIEW_3D' for space in area.spaces if
         space.type == 'VIEW_3D'))
    ...
    for ix, position in enumerate(
            [Matrix.Rotation(i, 4, 'Z') * Matrix.Rotation(j, 4, 'Y') * Matrix.Rotation(k, 4, 'X') for i in
             range(-45, 46, 45) for j in range(-45, 46, 45) for k in range(-45, 46, 45)]):
